...
/** Handheld: a strip under the controls. Tap to insert. */
export function MobileShelf() {
  const { available, state, insertCart, setShell } = useDevice()
  return (
    <aside className="border-t-[3px] border-ink bg-paper px-3 pb-4 pt-3" aria-label="Cartridge shelf">
      <div className="mb-2 flex items-baseline justify-between">
        <h2 className="font-display text-base">Shelf</h2>
        <span className="font-tiny text-[8px] uppercase text-ink/70">Tap to insert</span>
      </div>
      <ul className="no-scrollbar flex gap-3 overflow-x-auto pb-1">
        {available.map((c) => {
          const inSlot = state.cart.inserted === c.id
          return (
            <li key={c.id} className="shrink-0">
              <button
                ...
                aria-label={`${c.title}: ${c.tag}. ${inSlot ? 'In the slot.' : 'Insert cartridge.'}`}
                className="focus-ring block active:translate-y-[2px]"
                style={{ opacity: inSlot ? 0.4 : 1 }}
              >
                <Cartridge cart={c} width={96} sticker={c.bin === 'bargain'} />
              </button>
            </li>
          )
        })}
      </ul>
      <div className="mt-3 flex items-center gap-2" role="group" aria-label="Shell colour">
        <span className="font-tiny text-[8px] uppercase">Shell</span>
        {shellColors.map((c) => (
          <button
            key={c.value}
            ...
            aria-pressed={state.shell === c.value}
            aria-label={c.name}
            className="focus-ring h-6 w-6 rounded-sm border-2 border-ink aria-pressed:outline aria-pressed:outline-2 aria-pressed:outline-offset-2 aria-pressed:outline-ink"
            style={{ background: c.value }}
          />
        ))}
      </div>
